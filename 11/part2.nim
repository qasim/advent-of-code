...
proc printableImage(paintedPanelsTable: Table[Coordinate, BiggestInt]): string =
  var minX = high(int)
  var minY = high(int)
  ...
  for coordinate in paintedPanelsTable.keys():
  ...
  for y in minY .. maxY:
    var line: string
    for x in minX .. maxX:
      line &= $paintedPanelsTable.getOrDefault((x, y))
    lines.add(line)
...
proc main() =
  ...
  echo paintedPanelsTable.printableImage()
